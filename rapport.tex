...
\title{\textbf{TeachToLearn}\\[0.35em]\large Une salle de classe multi-agents pour apprendre en enseignant}
...
\begin{center}
\textbf{Url du projet~:} \url{http://myhandsomedns.ddns.net:5173/}
\\[0.3em]
\textbf{Dépôt GitHub~:} \url{https://github.com/clovislfbv/hackathon_scia}
\end{center}
...
\begin{center}
\begin{tabular}{@{}p{0.35\linewidth}@{\hspace{1.5em}}p{0.53\linewidth}@{}}
\toprule
\textbf{Fichier ou dossier} & \textbf{Description} \\
\midrule
\texttt{main.ts} & Scène Three.js et raccordement des modules \\
\texttt{scene/} & Salle, matériaux et éclairage \\
\texttt{objects/} & Bureau, chaises, tableau, porte, fenêtre et personnages \\
\texttt{player/} & Contrôles, collisions, interactions et phases du jeu \\
\texttt{chat/conversation.ts} & Chat, mémoires, Markdown, LaTeX et orchestration des agents \\
\texttt{vite.config.ts} & Endpoint \texttt{/api/chat}, prompts et streaming OpenAI \\
\texttt{player/objectives.ts} & Panneau d'objectifs et cases de validation \\
\bottomrule
\end{tabular}
\end{center}
